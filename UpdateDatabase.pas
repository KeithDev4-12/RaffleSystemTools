  ...
  while not VirtualTable1.EOF do begin
    with UMainModule do begin
      if VirtualTable1AccountNumber.AsString <> EmptyStr then begin
        qryMemberConsumers.Locate('AccountNumber',VirtualTable1AccountNumber.AsString,[])

      end else begin
        qryMemberConsumers.Filtered := False;
        qryMemberConsumers.Filter := 'Name Like ' + QuotedStr(StringReplace(VirtualTable1Name.AsString,',','',[rfReplaceAll, rfIgnoreCase]));
